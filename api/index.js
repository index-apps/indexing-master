...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (!clientEmail || !privateKey) {
            throw new Error("Missing Google Credentials in Vercel Environment Variables");
        }
        ...
        if (!oauthResponse.ok) {
            return res.status(401).json({ error: "Google Auth Failed: " + (oauthData.error_description || oauthData.error) });
        }
        ...
        if (indexingResponse.ok) {
            return res.status(200).json({ 
                message: 'Success! Google notified.', 
                data: indexingData 
            });
        } else {
            return res.status(indexingResponse.status).json({ 
                error: indexingData.error ? indexingData.error.message : 'Indexing API error' 
            });
        }
